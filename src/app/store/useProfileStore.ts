...
interface ProfileState {
  ...
  validateProfile: (dataToValidate?: ProfileFormData) => Record<string, string>;
...
export const useProfileStore = create<ProfileState>((set, get) => ({
  ...
  validateProfile: (dataToValidate?: ProfileFormData) => {
  ...
  setFormData: (data) => set((state) => {
    ...
    Object.keys(data).forEach((field) => {
      if (allErrors[field]) {
        if (allErrors[field].toLowerCase().includes("required") && !updatedForm[field as keyof ProfileFormData] && !state.isProfileSubmit && !state.formErrors[field]) {
  ...
  resetPasswordForm: () => set({ passwordData: initialPasswordData, passwordErrors: {} }),
}));
