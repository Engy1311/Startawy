...
interface AdminProfileState {
  ...
  setFormData: (data: Partial<AdminProfileFormData>) => void;
  ...
  validateProfile: (dataToValidate?: AdminProfileFormData) => Record<string, string>;
...
export const useAdminProfileStore = create<AdminProfileState>((set, get) => ({
  ...
  validateProfile: (dataToValidate?: AdminProfileFormData) => {
  ...
  setFormData: (data) => set((state) => {
    ...
    Object.keys(data).forEach((field) => {
      if (allErrors[field]) {
        if (allErrors[field].toLowerCase().includes("required") && !updatedForm[field as keyof AdminProfileFormData] && !state.isProfileSubmit && !state.formErrors[field]) {
